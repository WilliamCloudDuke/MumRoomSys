...
public interface StudentDao extends PagingAndSortingRepository<Student, Integer> {
	@Query("select s from Student s left join s.account a join a.role r where s.name like %:name%")
	public Page<Student> findByNameLike(@Param("name") String name, Pageable pageable);
	
	@Query("select s from Student s left join s.account a join a.role r where s.email like :email%")
